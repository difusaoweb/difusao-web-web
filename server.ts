...
async function createServer(isProd = process.env.NODE_ENV === 'production') {
  ...
  app.use('*', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // 3. Load the server main. vite.ssrLoadModule automatically transforms
      ...
      const devBuildPath = path.join(__dirname, './src/client/main-server.tsx')
      const { render } = await vite.ssrLoadModule(
        isProd ? productionBuildPath : devBuildPath
      )

      // 4. render the app HTML. This assumes main-server.js's exported `render`
      //    function calls appropriate framework SSR APIs,
      //    e.g. ReactDOMServer.renderToString()
      const appHtml = await render(url)
      const cssAssets = isProd ? '' : await stylesheets

      // 5. Inject the root-rendered HTML into the template.
      const html = template
